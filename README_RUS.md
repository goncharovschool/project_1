...
##Темы разбираемые в проекте:
1. Чтение кода на python и знакомство с ним;
2. Основы bash и написание простых bash-скриптов;
3. основные git-манипуляции.
...
3. В игре может быть другая игра, в которой будет еще одна. Это нормально. Все как в жизни. Рекурсия — это красиво.
4. Уровни могут сильно отличаться друг от друга. Это нормально. Это часть игры. Нельзя выучить один рецепт и его везде применять. Лишь непрерывно обучаясь и адаптируясь ты сможешь достигнуть цели. 
5. Наша игра — многопользовательская, даже если сначала тебе покажется иначе.
6. Хотя, большую часть пути ты сможешь преодолеть и один.
7. Будь внимателен к источникам информации. Проверяй. Думай. Анализируй. Сравнивай. Не доверяй.
8. Будь внимателен к тексту задания. Думай. Проверяй.
9. Если задание кажется непонятным или невыполнимым — это только так кажется. Просто посиди спокойно, в тишине, или включи любимую музыку. Вернись к заданию через 10-15 минут и перечитай его полностью. 
10. Если п.8 не помог — поищи проводника. Вокруг тебя — много таких же путников, как ты, они ждут тебя в нашем дискорд чате. Они с радостью помогут найти тебе выход.
11. Раз в неделю все интерисующие тебя вопросы ты можешь задать преподавателю.
12. Следи за временем! Оно коварно. Следи за дедлайнами.
13. Будь внимателен — и не упусти важное. Внимательно изучай репозиторий!
14. Всегда делай push только в ветку develop! Ветка master будет проигнорирована. Работай в директории src.
15. Помни, что каждое задание проходит ряд проверок: проверка на стиль кода, проверка статическим анализатором, проверка на корректную работу с памятью, проверка набором автотестов, проверка с помощью чеклиста. Будь внимателен. 
16. На твоем пути тебе встретятся разные задания. Те, что помечены звездочкой (*) — подходят только для самых отчаянных. Они с повышенной сложностью и в целом не являются обязательными к выполнению. Но если ты их сделаешь, то получишь дополнительный опыт и знания.
17. Иногда то, что кажется важным — не есть важное. 
18. Помни, в конечном счете, факт преодоления препятствия не так важен, как то, КАК ты его преодолел.
19. Главная цель нашего путешествия — осознать, что такое “КАК”.
20. Отделяй зерна от плевел.
21. Разделяй и властвуй. Декомпозируй. 
22. Думай о главном (о хорошем коде, разумеется). Следуй от общего к частному.
23. Не жульничай, не пытайся обмануть систему и окружающих. В первую очередь ты обманешь себя.
24. Не списывай, а если пользуешься помощью - всегда разбирайся до конца, почему, как и зачем. Иначе твое путешествие не будет иметь никакого смысла.  
25. Почаще заглядывай в папку materials. Там может быть много полезного!
26. Перечитай напутствия несколько раз.
...
lifehack. Если ты наплодил кучу файлов, которые не должны попасть в git-репозиторий. В папке src/ заведи папку ".gitignore". Любая папка название которой будет начинаться с точки, будет считаться скрытой. А имя gitignore зарезервировано git для игнорирования им же.
...
>Дверь ты можешь открыть скриптом src/ai_finaly_door.py, только вот, такая незадача *имитация сарказма*... генератор был поврежден. Вполне вероятно, что это был ты сам.

*Дальше много неразборчивого шума.* 

>Тебе нужно переписать условие таким образом, что
...
_**== Получен Quest 6. С помощью скрипта ai_finaly_door.py найди выход, запиши вторую версию кода отличающеюся от первой, но не меняющую ее суть. Поставь изменения под версионный контроль.  ==**_

***LOADING…***

## Quest 7. Python and bash. Закрепление.
Напиши скрипт на bash (hello.sh), который будет создавать файл hello.py. При запуске скрипта он добавляет команду вывода сообщения "hello world!". Запусти сгенерирированный файл. Положи изменения под версионный контроль.
